(function newInstance(window, document) {
  ...
  function getTemplateNodes(startingNode, templateUrl, $injector) {
    ...
    while ((node = node.nextSibling)) {
      ...
      if (node.nodeType === COMMENT_NODE && node.data === 'injular-end ' + templateUrl) {
        return nodes;
      }
    }
  ...
  function createInjularCommentWalker(templateUrl) {
    ...
    function filter(node) {
      if ((
          ...
          node.data === 'injular-start ' + templateUrl
          ) || (
          node.nodeType === 1 &&
          node.getAttribute('start-injular') === templateUrl)
          ) {
    ...
    function injularFilterRecipe(name, filterFactory) {
      ...
      if (!filtersCache) {
        throwError(
          ...
          'Are you sure angularFile property in injular options is correct?'
        );
      }
  ...
  function throwError(msg) {
    throw new window.Error('[Injular] ' + msg);
  }
    ...
    function logFn(logLevel) {
      ...
      function log() {
        if (logger.priority <= logLevelPriority) {
          ...
          window.Array.prototype.unshift.call(arguments, '[Injular]');
