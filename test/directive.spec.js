...
describe('directive injection', function() {
  ...
  it('should replace directive with injected one', function() {
    ...
    window.___injular___ = {directivesByUrl: {'/app/foo.directive.js': {foo: [fooDirective]}}};
  ...
  it('should be able to add a new directive with the same name as another existing directive', function() {
    ...
    function provide($provide, $compileProvider) {
      window.___injular___.$compileProvider = $compileProvider;
      provideRoute($provide);
      $compileProvider.directive('foo', valueFn(fooDirective));
    }
  });
  ...
  it('should add a new directive if a previous one didn\'t exist', function() {
    window.___injular___ = {directivesByUrl: {'/app/foo.directive.js': {}}};
    var element = angular.element('<div ng-app="app"></div>');
    rootElement.append(element);
    angular.bootstrap(element, ['app', provide]);
    ...
    expect(window.___injular___.directivesByUrl['/app/foo.directive.js'])
  ...
  it('should remove a directive if its file removed it', function() {
    ...
    window.___injular___ = {directivesByUrl: {'/app/foo.directive.js': {foo: directiveList}}};
    var element = angular.element('<div ng-app="app"></div>');
    rootElement.append(element);
    angular.bootstrap(element, ['app', provide]);
    element.injector().get('fooDirective');
  ...
  it('should remove a directive even if it was never initialized', function() {
    ...
    window.___injular___.addScriptUrlToDirectives(directiveUrl);
    ...
    var $injector = element.injector();
    expect($injector.get('fooDirective')).to.have.length(0);
    expect(window.___injular___.directivesByUrl[directiveUrl].foo)
    .to.have.length(1);
  });
  ...
  it('should not remove the following properties: index', function() {
    ...
    window.___injular___ = {directivesByUrl: {'/app/foo.directive.js': {foo: fooDirectives}}};
  ...
  it('should remove a directive related only to the modified file, if it was removed from it', function() {
    ...
    window.___injular___ = {directivesByUrl: {
  ...
  it('should handle undefined directive', function() {
    var fileDirectives = {};
    window.___injular___ = {directivesByUrl: {'/app/foo.directive.js': fileDirectives}};
  ...
  it('should add a directive when multiple directives with the same name but different files exist', function() {
    ...
    window.___injular___ = {directivesByUrl: directivesByUrl};
    var element = angular.element('<div ng-app="app"></div>');
    rootElement.append(element);
    angular.bootstrap(element, ['app', provide]);
  ...
  if (VERSION_MINOR >= 5) {
    it('should handle component recipe', function() {
      ...
      var $injector = element.injector();
      var fooDirectives = $injector.get('fooDirective').slice(); // slice for copy
      window.___injular___.directivesByUrl = {'/app/foo.component.js': {foo: fooDirectives}};
